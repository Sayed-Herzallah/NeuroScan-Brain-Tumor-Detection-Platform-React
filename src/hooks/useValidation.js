...
export function validatePassword(password) {
  ...
  if (!PWD_REGEX.test(password)) return "Password must contain uppercase, lowercase, number and special character";
  return "";
}
...
export function validatePhone(phone) {
  if (!phone) return "";
  if (!PHONE_REGEX.test(phone)) return "Enter a valid Egyptian (010/011/012/015XXXXXXXX) or Saudi (966XXXXXXXXX) number";
  return "";
}
...
export function useForm(initialValues) {
  ...
  const handleBlur = useCallback((field, validatorFn) => {
    ...
    if (validatorFn) {
      const err = validatorFn(values[field]);
      setErrors(e => ({ ...e, [field]: err || "" }));
    }
  }, [values]);

  const setError = useCallback((field, msg) => setErrors(e => ({ ...e, [field]: msg })), []);
